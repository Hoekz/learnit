# Writing a Course with Learn It

This guide is the ultimate goal for `learnit` features, it does not accurately reflect the
...
CLI provides all the tools you should need to create a course. If you make a mistake in the course,
currently the best approach is to use `git` directly for actions such as rebasing.
...
```
learnit new module <module_name> [--cwd=<directory>]
```

A branch will be created for the module called `module-<module_name>` where `module_name` has been
cast to lowercase and spaces converted to dashes (`-`). For example, `My Module` produces `module-my-module`.
...
```
learnit new chapter [chapter_title]
```

It is highly recommended that you name your chapters for clarity during navigation. Chapters are
automatically incremented up from the last, starting at `01`. You must be currently
